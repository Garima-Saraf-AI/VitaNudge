...
- Name: quick-smoke-test.spec.js >> VitaNudge - Quick Smoke Test Suite >> AUTH-003: Invalid email validation
- Location: tests/quick-smoke-test.spec.js:57:7
...
Call log:
  - waiting for locator('input[placeholder*="name"], input[name="name"]')
...
> 62  |     await page.fill('input[placeholder*="name"], input[name="name"]', 'Test User');
  ...
  139 |       console.log('Console errors found:', consoleErrors);
  140 |     }
  141 | 
  142 |     // Allow 0 errors (or log them for review)
  143 |     console.log(`Console errors: ${consoleErrors.length}`);
  144 | 
  145 |     console.log('✅ UI-001 PASSED - Console errors logged');
  146 |   });
  147 | });
  148 | 
```